...
from models.sinet_eca import SINet_ECA
...
PRED_DIR = "pred_samples_sinet_eca"
...
# Model
# -----------------------
model = SINet_ECA(pretrained=True).to(device)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if avg_dice > best_val_dice:
        ...
        torch.save(model.state_dict(), os.path.join(SAVE_DIR, "sinet_eca_best.pth"))
        print(" Saved best model (by Dice):", os.path.join(SAVE_DIR, "sinet_eca_best.pth"))
...
plt.ylabel("Loss")
plt.legend()
plt.title("SINet + ECA Training Curve")
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, "sinet_eca_loss_curve.png"))
print(" Saved loss curve:", os.path.join(SAVE_DIR, "sinet_eca_loss_curve.png"))
...
model.load_state_dict(
    torch.load(
        os.path.join(SAVE_DIR, "sinet_eca_best.pth"),
        map_location=device,
        weights_only=True
    )
)
